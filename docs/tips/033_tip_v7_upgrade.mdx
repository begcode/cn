...
我们为 HTML 启用了 Prettier。你可以在升级之前运行启用 HTML 的 Prettier，并将更改提交到 Git，以便在升级后查看 HTML 文件中的真实更改。

## **Angular 客户端中的翻译指令更改**

我们已将 `jhiTranslate` 指令从 [ng-jhipster](https://github.com/jhipster/ng-jhipster/blob/main/src/language/jhi-translate.directive.ts) 移动到生成的应用程序中。如果你的 `jhiPrefix` 与 `jhi` 不同，那么你可能希望在升级之前将所有 `jhiTranslate` 替换为 `yourprefixTranslate` 并将更改提交到 Git，以减少升级后显示的更改。

## **Angular 客户端中重命名的文件**

在 Angular 客户端中，许多文件被重命名，测试被移动到它们正在测试的文件旁边。

在[手动升级](/upgrading-an-application/#option-2---manual-upgrade)说明页面中有**关于重命名文件的提示**部分 - 这可以帮助升级。

如果前面的提示没有给出令人满意的结果，那么你可以在升级之前执行以下操作：
* 将重命名的文件移动到它们的最终位置
* 之后提交更改
