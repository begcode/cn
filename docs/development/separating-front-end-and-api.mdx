...
JHipster 是一个“全栈”开发工具，它的目标是让您高效地处理前端代码（Angular/React）和后端代码（Spring Boot）。

然而，分离前端和后端代码是一个常见的需求，通常是因为它们由不同的团队开发并具有不同的生命周期。

**请注意**，这不是默认的 JHipster 工作方式：这样做并不复杂，并且效果很好，但这是一个高级主题。如果您刚开始使用 JHipster，我们建议您先使用我们的标准工作方式。
